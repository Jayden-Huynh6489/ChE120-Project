...
ROW_COUNT = 6#initize row variable
COLUMN_COUNT = 7#initize column variable
 
 """Here, a function for creating the playing board is being defined. The board is made using the module numpy as np, and the previous variables 
 corresponding to the number of rows and columns."""
def create_board():
    """this function is to in initize a board. at first
...
def print_board(board):
 ...
 """This function is meant to check if there is 4 in a row horizontally. It checks if there are 4 pieces in a row from an initial position c (that's
 why there are things like c+1, c+2, c+3). r must be constant for this to be the case because it only counts horizontally. If there are 4 in a row horizontally, 
 then True is returned and there is a winner."""
def winning_move(board, piece):
 ...
 """This function acts like the previous one, except this time it check if there are 4 in a row vertically. There must be 4 pieces from an initial position r,
 and then r+1, r+2, r+3 all must follow. c is constant because only the rows are being counted, not the columns True is returned if there are 4 in a row
 vertically and there is a winner."""
 ...
 To do this it counts one to the left or right from c, and then one up from row. If there are 4 in row with a positive slope, then True is returned and there
 is a winner."""
